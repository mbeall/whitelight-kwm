...
class Woo_BlogAuthorInfo extends WP_Widget {
  ...
  function widget( $args, $instance ) {
    ...
    if ( ( $page == 'home' && is_home() ) || ( $page == 'single' && is_single() ) || $page == 'all' ) {
      ...
      <span class="<?php echo $avatar_align; ?>"><?php
        if ( $custom_email ) {
          echo get_avatar( $custom_email, $size = $avatar_size );
        } ?>
      </span>
      <p><?php echo $bio; ?></p><?php

      if ( $read_more_url ) {
        echo '<p><a href="' . esc_url( $read_more_url ) . '">' . esc_html( $read_more_text ) . '</a></p>';
      } ?>
